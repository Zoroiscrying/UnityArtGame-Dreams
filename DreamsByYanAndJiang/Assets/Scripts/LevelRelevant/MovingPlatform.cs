...
namespace Hertzole.GoldPlayer.Example
{
    ...
    public class MovingPlatform : MonoBehaviour
    {
        ...
        void Start()
        {
            ...
            List<Waypoint> worldWayPoints = new List<Waypoint>();
            foreach (var point in waypoints)
            {
               var newPos = point.position + this.transform.position;
               var newRot = point.rotation + this.transform.rotation.eulerAngles;
               var newWaypoint = new Waypoint();
               newWaypoint.position = newPos;
               newWaypoint.rotation = newRot;
               newWaypoint.waitTime = point.waitTime;
               worldWayPoints.Add(newWaypoint);
            }
        ...
        void OnDrawGizmosSelected()
        {
            ...
            if (waypoints != null)
            {
                if (waypoints.Count > 0)
                {
                    ...
                    {
                        Gizmos.DrawCube(this.transform.position + pos.position, new Vector3(0.5f, 0.5f, 0.5f));
                    }
                    ...
                    for (int i = 0; i < waypoints.Count; i++)
                    {
                        ...
                        {
                            Gizmos.DrawLine(this.transform.position +waypoints[i].position, this.transform.position +waypoints[0].position);
                        }
                        else
                        {
                            Gizmos.DrawLine(this.transform.position +waypoints[i].position, this.transform.position +waypoints[i + 1].position);
                        }
                    }
                }
            }
